	...
	if (err) throw err;
			...
			function findMinAndMax(dataColumnX) {
				...
				yMax = d3.max(mydata, function(data) {
					return +data.NotAffordHealthcare * 1.5; //LG is this because it's the default y?
				});

				yMin = d3.min(mydata, function(data){
					return +data.NotAffordHealthcare * 0.3;  //LG is this because it's the default y?

				});
			}

			// The default x-axis is 'lessthanHighscool'  LG Change this part
		...
		//3rd chart - NoHealthcoverage && depression_datavalue
			var toolTip = d3.tip()
					...
					.html(function(data) {
						...
						else if (currentAxisLabelX === 'NoHealthcoverage') {
						...
						else if (currentAxisLabelY === 'depression_datavalue'){
